...
1. Search for `'central'` (inside single quotations) to capture the text domain.
2. Search for `central_` to capture all the function names.
3. Search for <code>&nbsp;_tk</code> (with a space before it) to capture DocBlocks.
4. Search for `central-` to capture prefixed handles.

OR

* Search for: `'central'` and replace with: `'megatherium'`
* Search for: `central_` and replace with: `megatherium_`
